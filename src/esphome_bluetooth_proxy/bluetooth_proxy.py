...
from .ble_connection import BLEConnection, ConnectionState
from .device_info import BluetoothProxyFeature
...
class BluetoothProxy:
    ...
    def _initialize_connection_pool(self) -> None:
        ...
        logger.debug(f"Initialized connection pool with {len(self.connection_pool)} slots")
    
    async def subscribe_api_connection(self, api_connection: APIConnection, flags: int) -> None:
    ...
    async def _start_scanning(self) -> None:
        ...
        try:
            await self.scanner.start_scanning(active=False)  # Start with passive scanning
    ...
    def _send_advertisement_batch(self, advertisements: List[BLEAdvertisement]) -> None:
        ...
        for adv in advertisements:
            logger.debug(f"Advertisement: {adv.address:012X} RSSI={adv.rssi} "
    ...
    async def _send_scanner_state(self, api_connection: APIConnection) -> None:
        ...
        logger.debug(f"Sending scanner state to {api_connection} (scanning={self.scanning_enabled})")
    ...
    async def connect_device(self, address: int, address_type: int) -> bool:
        ...
        available_connection.address_str = available_connection._address_to_string(address)
    ...
    async def on_device_connected(self, address: int, connected: bool, mtu: int = 0, error: str = "") -> None:
        ...
        """
        logger.info(f"Device {address:012X} connection state: connected={connected} MTU={mtu}")

...
class BLEConnection:
    ...
    async def connect(self) -> bool:
        ...
        if self.state != ConnectionState.DISCONNECTED:
            logger.warning(f"Connection to {self.address_str} already in progress or connected")
            return False
    ...
    async def discover_services(self) -> List[BLEService]:
        ...
        try:
            ...
            for service in bleak_services:
                ...
                for char in service.characteristics:
                    ...
                    for desc in char.descriptors:
                        descriptors.append(BLEDescriptor(
                            uuid=self._uuid_to_bytes(desc.uuid),
                            handle=desc.handle
                        ))
                    
                    characteristics.append(BLECharacteristic(
                ...
                self.services.append(BLEService(
            ...
            logger.info(f"Discovered {len(self.services)} services for {self.address_str}")
    ...
    async def read_characteristic(self, handle: int) -> bytes:
        ...
        try:
            ...
            logger.debug(f"Read {len(value)} bytes from handle {handle} on {self.address_str}")
            return value
            
        except Exception as e:
            logger.error(f"Failed to read characteristic {handle} on {self.address_str}: {e}")
            raise
    ...
    async def write_characteristic(self, handle: int, data: bytes, response: bool = True) -> bool:
        ...
        try:
            ...
            logger.debug(f"Wrote {len(data)} bytes to handle {handle} on {self.address_str}")
            return True
            
        except Exception as e:
            logger.error(f"Failed to write characteristic {handle} on {self.address_str}: {e}")
            return False
    
    def _find_characteristic_by_handle(self, handle: int) -> Optional[BleakGATTCharacteristic]:

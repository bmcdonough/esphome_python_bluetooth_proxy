...
class PairingManager:
    ...
    async def pair_device(self, address: int) -> bool:
        ...
        try:
            ...
            if not connection:
                ...
                await self._send_pairing_response(address, False, "Device not connected")
                return False
    ...
    async def _send_pairing_response(self, address: int, success: bool, error: str = "") -> None:
        ...
        logger.debug(f"Sending pairing response: device={address:012X} success={success} error={error}")
    
    async def _send_unpairing_response(self, address: int, success: bool, error: str = "") -> None:
        ...
        """
        # TODO: Implement protobuf message sending
        logger.debug(f"Sending unpairing response: device={address:012X} success={success} error={error}")
    
    async def _send_cache_clear_response(self, address: int, success: bool, error: str = "") -> None:
        ...
        """
        # TODO: Implement protobuf message sending
        logger.debug(f"Sending cache clear response: device={address:012X} success={success} error={error}")
    ...
    def get_stats(self) -> dict:
        ...
        """
        paired_count = sum(1 for state in self.pairing_states.values() 

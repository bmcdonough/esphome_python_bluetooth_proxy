...
from .protocol import (
    MessageType, MessageEncoder, MessageDecoder, ProtocolError,
    create_message_frame, parse_message_frame,
    HelloRequest, HelloResponse, ConnectRequest, ConnectResponse,
    DeviceInfoRequest, DeviceInfoResponse, ListEntitiesRequest, ListEntitiesDoneResponse
)
...
class APIConnection:
        ...
        while not self.writer.is_closing():
            try:
                ...
                while len(buffer) >= 3:  # Minimum frame size
                    try:
                        msg_type, payload, frame_size = parse_message_frame(bytes(buffer))
                        ...
                        if "Incomplete message frame" in str(e):
                            ...
                            logger.error(f"Protocol error from {self.client_address}: {e}")
                            return
    ...
    async def _handle_message(self, msg_type: int, payload: bytes) -> None:
        ...
        try:
            if msg_type == MessageType.HELLO_REQUEST:
                ...
                logger.warning(f"Unhandled message type {msg_type} from {self.client_address}")
                
        except Exception as e:
            logger.error(f"Error handling message type {msg_type} from {self.client_address}: {e}")
    
    async def _handle_hello_request(self, payload: bytes) -> None:
        """Handle HelloRequest message."""
        if self.state != ConnectionState.CONNECTING:
            logger.warning(f"Unexpected HelloRequest from {self.client_address} in state {self.state}")
            return
        ...
        try:
            ...
            logger.info(f"Hello from {self.client_address}: '{request.client_info}' "
            ...
            await self._send_message(MessageType.HELLO_RESPONSE, 
                                   self.encoder.encode_hello_response(response))
            ...
            if self.password is None:
                ...
                logger.debug(f"Client {self.client_address} connected, waiting for authentication")
    ...
    async def _handle_connect_request(self, payload: bytes) -> None:
        ...
        if self.state != ConnectionState.CONNECTED:
            logger.warning(f"Unexpected ConnectRequest from {self.client_address} in state {self.state}")
            return
        ...
        try:
            ...
            # Check password
            password_valid = (self.password is None) or (request.password == self.password)
            
            response = ConnectResponse(invalid_password=not password_valid)
            await self._send_message(MessageType.CONNECT_RESPONSE,
            ...
            if password_valid:
                ...
                logger.warning(f"Client {self.client_address} provided invalid password")
                await self.close()
                
        except Exception as e:
            logger.error(f"Error handling ConnectRequest from {self.client_address}: {e}")
    
    async def _handle_device_info_request(self, payload: bytes) -> None:
        """Handle DeviceInfoRequest message."""
        if self.state != ConnectionState.AUTHENTICATED:
            logger.warning(f"DeviceInfoRequest from unauthenticated client {self.client_address}")
            return
        ...
        try:
            ...
            await self._send_message(MessageType.DEVICE_INFO_RESPONSE,
                                   self.encoder.encode_device_info_response(device_info))
            
            logger.debug(f"Sent device info to {self.client_address}")
            
        except Exception as e:
            logger.error(f"Error handling DeviceInfoRequest from {self.client_address}: {e}")
    
    async def _handle_list_entities_request(self, payload: bytes) -> None:
        """Handle ListEntitiesRequest message."""
        if self.state != ConnectionState.AUTHENTICATED:
            logger.warning(f"ListEntitiesRequest from unauthenticated client {self.client_address}")
            return
        ...
        try:
            ...
            await self._send_message(MessageType.LIST_ENTITIES_DONE_RESPONSE,
                                   self.encoder.encode_list_entities_done_response(response))
            
            logger.debug(f"Sent entity list to {self.client_address}")
            
        except Exception as e:
            logger.error(f"Error handling ListEntitiesRequest from {self.client_address}: {e}")
    ...
    async def _send_message(self, msg_type: int, payload: bytes) -> None:
        ...
        try:
            ...
            logger.debug(f"Sent message type {msg_type} to {self.client_address} "
    ...
    async def send_message(self, msg_type: int, payload: bytes) -> None:
        ...
        if self.state != ConnectionState.AUTHENTICATED:
            logger.warning(f"Attempt to send message to unauthenticated client {self.client_address}")
            return

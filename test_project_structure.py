...
def test_imports():
    ...
    try:
        ...
        from esphome_bluetooth_proxy import (
            ESPHomeAPIServer, APIConnection, ConnectionState,
            DeviceInfoProvider, BluetoothProxyFeature,
            MessageType, MessageEncoder, MessageDecoder
        )
        print("✓ Phase 1 imports successful")
        
        # Phase 2: BLE Scanning Components
        from esphome_bluetooth_proxy import (
            BLEScanner, BLEAdvertisement, AdvertisementBatcher
        )
        print("✓ Phase 2 imports successful")
        
        # Phase 3: BLE Connection Components
        from esphome_bluetooth_proxy import (
            BLEConnection, BLEService, BLECharacteristic, BLEDescriptor
        )
        print("✓ Phase 3 imports successful")
        
        # Phase 4-6: GATT Operations and Advanced Features
        from esphome_bluetooth_proxy import (
            GATTOperationHandler, PairingManager, PairingState
        )
...
def test_basic_functionality():
    ...
    try:
        from esphome_bluetooth_proxy import (
            DeviceInfoProvider, BluetoothProxyFeature, ESPHomeAPIServer
        )
        ...
        if device_info.bluetooth_proxy_feature_flags == expected_flags:
            ...
            print(f"✗ Feature flags mismatch: expected 0x{expected_flags:02x}, "
...
def test_class_structure():
    ...
    try:
        from esphome_bluetooth_proxy import (
            ESPHomeAPIServer, BluetoothProxy, BLEScanner, 
            BLEConnection, GATTOperationHandler, PairingManager
        )
        
        # Test ESPHomeAPIServer
        server = ESPHomeAPIServer()
        expected_methods = ['start', 'stop', 'get_authenticated_connections', 'broadcast_message']
        ...
        expected_methods = ['start_scanning', 'stop_scanning', 'set_scan_mode', 'is_scanning']
        for method in expected_methods:
        ...
        expected_methods = ['connect', 'disconnect', 'discover_services', 'read_characteristic', 'write_characteristic']
        for method in expected_methods:
